...
## BUG-003 — Edge OAuth not working (getAuthToken not supported in sidepanel/setup)
**Status:** Resolved
**Date:** 2026-03-14
...
**Fix:** Added cross-browser getOAuthToken() helper in utils/helpers.js that detects Chrome vs Edge. Requires separate Web application OAuth client in Google Cloud Console with chromiumapp.org redirect URI.
...
**Root cause:** Google Drive folders created with old Drive sync client have legacy 0B... format IDs. These cannot be listed as children via Drive API v3 — returns 404. This is a Google limitation.
**Fix:** Added 404 fallback with supportsAllDrives=true. Changed scope to drive.readonly. Full-height folder picker UI.
**Workaround:** Move affected folders in Google Drive (right-click → Move to → same location) to re-parent with modern ID.
...
**Root cause:** Storage keys existed in setup.js but not registered in STORAGE_KEYS in helpers.js. Page load used live Drive API call to restore names which failed silently.
**Fix:** Added all keys to STORAGE_KEYS and DEFAULT_STORAGE. Names saved at save time and restored from storage — no live Drive API call.
...
**Status:** Resolved
**Date:** 2026-03-15
**Symptom:** Gemini model dropdown showed 1.5 Flash and 1.5 Pro — retired by Google
...
**Status:** Known issue — deferred pre-launch
**Date:** 2026-03-15
**Root cause:** handleActivateLicence() in setup/setup.js accepts any key without server validation (V1 stub)
